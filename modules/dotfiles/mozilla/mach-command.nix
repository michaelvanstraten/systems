...
let
  cfg = config.programs.mach;
in
{
  options.programs.mach = {
    enable = lib.mkEnableOption "mach wrapper that finds the nearest ./mach and forwards args";
    enableFishCompletion = lib.mkEnableOption "install fish completion for the mach wrapper";
  };
  ...
  config = lib.mkIf cfg.enable (
    let
      ...
      mach = pkgs.symlinkJoin {
        ...
        ++ lib.optionals cfg.enableFishCompletion [ fishCompletion ];
      };
    in
    {
      home.packages = [ mach ];
